...
fn main() -> Result<()> {
    ...
    if target_os == "macos" && target_family != "wasm" {
        ...
        let target_dir =
            app_target_dir_for_build(&profile).expect("Failed to get app target directory");
    ...
    if target_os == "windows" {
        ...
        let target_dir = app_target_dir_for_build(&cargo_full_profile)
...
fn app_target_dir_for_build(profile: &str) -> Result<PathBuf> {
    let target_dir = app_target_dir(profile)?;

    if let Ok(target_triple) = env::var("TARGET") {
        if target_triple != env::var("HOST").unwrap_or_default() {
            if let Some(profile_dir) = target_dir.file_name() {
                if let Some(workspace_target_dir) = target_dir.parent() {
                    return Ok(workspace_target_dir.join(target_triple).join(profile_dir));
                }
            }
        }
    }
