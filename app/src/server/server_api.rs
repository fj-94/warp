...
impl ServerApi {
    ...
    ) -> std::result::Result<AIOutputStream<warp_multi_agent_api::ResponseEvent>, Arc<AIApiError>>
    {
        let auth_token = match self.get_or_refresh_access_token().await {
            Ok(token) => token,
            Err(err) if Self::request_has_user_inference_credentials(request) => {
                log::info!(
                    "Proceeding with an unauthenticated agent request using user-provided inference credentials."
                );
                AuthToken::NoAuth
            }
    ...
    fn request_has_user_inference_credentials(request: &warp_multi_agent_api::Request) -> bool {
        let Some(settings) = request.settings.as_ref() else {
            return false;
        };

        let has_byo_key = settings.api_keys.as_ref().is_some_and(|api_keys| {
        ...
        has_byo_key || has_custom_endpoint
    }
