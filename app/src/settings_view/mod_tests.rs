...
fn filtered_out_top_level_page_is_skipped() {
    ...
    // But other nav stops remain.
    assert!(stops
        .iter()
        .any(|s| matches!(s, NavStop::CollapsedUmbrella { nav_index: 0, .. })));
}
...
#[test]
fn arrow_down_wraps_from_last_collapsed_umbrella_to_first_subpage() {
    let nav_items = realistic_nav_items();
    let stops = build_nav_stops(&nav_items, |_| true);

    // Pressing Down from the final collapsed umbrella should wrap to Agents
    // and select WarpAgent.
...
fn arrow_down_collapsed_umbrella_respects_search_filter() {
    ...
    // From the final collapsed umbrella, Down wraps to Agents and lands on
    // AgentMCPServers (first visible subpage), not on WarpAgent / AgentProfiles.
    let next = simulate_cycle(
        &nav_items,
        &stops,
        SettingsSection::OzCloudAPIKeys,
