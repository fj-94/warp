...
impl Task {
    ...
    pub(super) fn add_messages(
        ...
        if let TaskImpl::Server(ServerTask { source, .. }) = &mut self.data {
            source.messages.extend(messages);
        } else {
            log::info!(
                "Added messages to optimistic AI task exchange without server task source: task_id={}",
                self.id
            );
        }
        Ok(())
