    ...
    App::test((), |mut app| async move {
        ...
        app.read(|ctx| {
            assert!(
                ...
                "expected is_byo_api_key_enabled to be true for anonymous Firebase users",
            );
        });
        ...
        request_usage_model.update(&mut app, |model, ctx| {
            ...
            assert!(
                model.has_any_ai_remaining(ctx),
                "expected has_any_ai_remaining to be true for anonymous Firebase user with a BYO key",
            );
        });
    });
}
